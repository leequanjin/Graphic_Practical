...
float bridgeAngle = 0.0f;   // rotation angle for raising
float bridgeSpeed = 1.0f;   // speed of raising/lowering
...
LRESULT WINAPI WindowProcedure(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		else if (wParam == 'N') { // Raise bridge
			if (bridgeAngle > -40) bridgeAngle -= bridgeSpeed;
			}
		else if (wParam == 'M') { // Lower bridge
			if (bridgeAngle < 0) bridgeAngle += bridgeSpeed;
		}
		break;
...
void drawPyramid(float base, float height) {
	...
	// Front face
	glTexCoord2f(0.0f, 0.0f); glVertex3f(-half, 0, half);
	glTexCoord2f(1.0f, 0.0f); glVertex3f(half, 0, half);
	glTexCoord2f(0.5f, 1.0f); glVertex3f(0, height, 0);
	...
	// Base (optional, if you want to see the bottom covered)
	glBegin(GL_QUADS);
	glTexCoord2f(0.0f, 0.0f); glVertex3f(-half, 0, half);
	glTexCoord2f(1.0f, 0.0f); glVertex3f(half, 0, half);
	glTexCoord2f(1.0f, 1.0f); glVertex3f(half, 0, -half);
	glTexCoord2f(0.0f, 1.0f); glVertex3f(-half, 0, -half);
	glEnd();
}
...
void drawBridge() {
	...
	glPushMatrix();
	glTranslatef(-8, 5, 0);
	textureArr[0] = loadTexture("tower.bmp");
	drawCube(4, 12, 4);
	glDeleteTextures(1, &textureArr[0]);
		glPushMatrix();
			glTranslatef(-4, -7, 0);
			textureArr[2] = loadTexture("metal_layered.bmp");
			drawCube(14, 5, 5);
			glDeleteTextures(1, &textureArr[2]);
		glPopMatrix();
		glPushMatrix();
			glTranslatef(0, 6, 0);
			textureArr[3] = loadTexture("roof_tile.bmp");
			drawPyramid(4, 5);
			glDeleteTextures(1, &textureArr[3]);
		glPopMatrix();
	glPopMatrix();
	...
	// --- Right bridge deck (rotating) ---
	glPushMatrix();
	glTranslatef(7, 0, 0);   // move pivot to hinge
	glRotatef(bridgeAngle, 0, 0, 1); // rotate upwards
	glTranslatef(-4, 0, 0);   // move back
	textureArr[1] = loadTexture("Box.bmp");
	drawCube(6, 1, 4);
	glPopMatrix();

	// --- Left bridge deck (rotating) ---
	glPushMatrix();
	glTranslatef(-7, 0, 0);
	...
	textureArr[1] = loadTexture("sky.bmp");
	drawCube(80, 80, 1);
	glPopMatrix();
	glDeleteTextures(1, &textureArr[1]);
	glPopMatrix();
	...
	glPushMatrix();
	glTranslatef(0, -8, 0);
	textureArr[1] = loadTexture("storm.bmp");
	drawCube(80, 10, 50);
	glPopMatrix();
	glDeleteTextures(1, &textureArr[1]);
	glPopMatrix();
